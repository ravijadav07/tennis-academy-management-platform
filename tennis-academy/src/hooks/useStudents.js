  ...
  const enriched = useMemo(() => {
    return students
      ...
      .map((s) => {
        ...
        const batchIds = [...new Set(enr.map((e) => e.batchId))];
        return { ...s, enrollments: enr, batch, packages: allPkgs, package: pkg, eligibility, programs, batchIds, isAmbiguous: programs.length > 1 };
      })
      .filter((s) => {
