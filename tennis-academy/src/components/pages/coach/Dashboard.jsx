...
export default function CoachDashboard() {
  ...
  const todayData = useMemo(() => {
    ...
    const privSessions = (state.privateSessions || []).filter((s) => s.coachId === coachId && (s.date === today || s.dayPattern === todayPattern));
  ...
  }, [state, coachId, today, todayPattern]);
  ...
  const myStudents = state.enrollments.filter((e) => {
    ...
    return b && (b.primaryCoachId === coachId || b.supportCoachId === coachId) && b.status === 'ACTIVE';
  }).length;
    ...
    <div className="space-y-4">
      ...
      <Card>
        ...
        {todayData.batches.length === 0 && todayData.privateSessions.length === 0 ? (
          <p className="text-xs text-ink-muted py-2">No {todayPattern} batches or private sessions scheduled for you today.</p>
        ) : (
          <div className="space-y-2">
            {todayData.batches.map((b) => (
              <div key={b.id} className="flex items-center justify-between gap-3 px-3 py-2.5 rounded-lg bg-canvas-soft text-xs">
                <span className="font-semibold text-ink">{getBatchDisplayName(b, state.courts)}</span>
