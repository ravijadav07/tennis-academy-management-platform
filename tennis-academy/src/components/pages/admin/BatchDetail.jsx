      ...
      <div className="space-y-4">
        ...
        <Card>
          <div className="space-y-3">
            ...
            {/* Batch Details Group — cascading order: Court → Day Pattern → Timing → Capacity → Coaches */}
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <Sel
                  ...
                  options={courtsForCategory(courts, s.batches, nf.program).map((c) => ({ value: c.id, label: c.name }))}
                  disabled={!catResolved(nf)}
                  emptyOption={!catResolved(nf) ? 'Select Category first' : 'Select Court...'}
                />
                <Sel
                  label="Day Pattern"
                  value={nf.dayPattern}
              ...
              <div className="grid grid-cols-2 gap-3">
                <Field label="Start Time *" type="time" value={nf.startTime} onChange={(v) => setNf((f) => ({ ...f, startTime: v }))} disabled={!nf.courtId} />
                <Field label="End Time" type="time" value={nf.endTime} onChange={(v) => setNf((f) => ({ ...f, endTime: v }))} disabled={!nf.courtId} />
              </div>
              <div className="grid grid-cols-2 gap-3">
          ...
          {roster.map((e) => {
            ...
            return (
              <div key={e.id} className={'flex items-center gap-3 px-3 py-2 rounded-lg text-xs ' + (blocked ? 'bg-err-bg/30 opacity-60' : 'bg-canvas-soft')}>
                ...
                {st && st.membershipType === 'Guest' ? <StatusPill status="guest" /> : null}
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <Sel
                ...
                options={courtsForCategory(courts, s.batches, ef.program).map((c) => ({ value: c.id, label: c.name }))}
                disabled={!catResolvedEf(ef)}
                emptyOption={!catResolvedEf(ef) ? 'Select Category first' : 'Select Court...'}
              />
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="Start Time" type="time" value={ef.startTime || ''} onChange={(v) => setEf((f) => ({ ...f, startTime: v }))} disabled={!ef.courtId} />
              <Field label="End Time" type="time" value={ef.endTime || ''} onChange={(v) => setEf((f) => ({ ...f, endTime: v }))} disabled={!ef.courtId} />
            </div>
            <div className="grid grid-cols-2 gap-3">
